...
async function handleApi(request, response, url) {
    if (request.method === 'POST' && url.pathname === '/api/auth/login') {
        ...
        const user = { id: staff.id, employeeId: staff.employee_id, name: staff.name, role: staff.role, department: staff.department, designation: staff.designation };
    ...
    if (url.pathname === '/api/staff' && request.method === 'GET') {
        ...
        return json(response, 200, { staff: database.prepare('SELECT id, employee_id AS employeeId, name, department, designation, login_id AS loginId, role, status, manager_id AS managerId, created_at AS joiningDate FROM staff ORDER BY name').all() });
    }
